...
import { X, MoreHorizontal, Heart, Send, Smile, Trash2, Music, Volume2, VolumeX } from 'lucide-react';
    ...
    <div className="fixed inset-0 z-[300] bg-black flex flex-col font-sans">
      ...
      <div className="flex-1 relative w-full h-full flex items-center justify-center bg-[#121212] overflow-hidden" onPointerDown={startHoldingStory} onPointerUp={stopHoldingStory} onPointerCancel={stopHoldingStory}>
        ...
        {currentStory.music_url && (
          <div className="absolute bottom-20 left-4 right-4 z-20 flex items-center justify-center pointer-events-none">
            <div className="bg-black/50 backdrop-blur-md rounded-full px-4 py-2.5 text-white text-sm font-bold border border-white/20 flex items-center gap-2 animate-in fade-in">
              <Music className="w-4 h-4 text-yellow-400" /> <span className="max-w-[70vw] truncate">{currentStory.music || 'Music'}</span>
              <button onClick={() => { if (audioRef.current) audioRef.current.muted = !isMusicMuted; setIsMusicMuted(!isMusicMuted); }} className="pointer-events-auto text-white hover:text-yellow-400 transition-colors" aria-label="Toggle music">
